/**
 * Roll Ferocity from an on-use Item Macro as controlled by MidiQOL.
 * For Foundry v10.
 *
 * @version 10
 */
...
const itmFerocity = actor.items.find(it => it.name === "Ferocity");
...
await ChatMessage.create({
	speaker: {actor: actor},
	content: `<div>
			<p>${actor.name} is at risk of entering a rampage!</p>
			<p>${game.user.character.name} must succeed on a DC ${5 + totalFerocity} Wisdom (Animal Handling) check (no action required), or ${actor.name} moves up to their speed toward the nearest creature
